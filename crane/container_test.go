...
func TestDependencies(t *testing.T) {
	...
	if deps := container.Dependencies(); deps.all[0] != "a" || deps.all[1] != "b" || deps.all[2] != "c" || deps.all[3] != "n" || deps.link[0] != "a" || deps.link[1] != "b" || deps.all[2] != "c" || deps.net != "n" {
	...
	if deps := container.Dependencies(); len(deps.all) != 0 && len(deps.link) != 0 && deps.net == "" {
